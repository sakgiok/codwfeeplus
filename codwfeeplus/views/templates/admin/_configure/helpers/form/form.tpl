					...
					{foreach $field as $input}
						...
						{else}
							...
							{block name="field"}
								...
								{elseif $input.type == 'switch_custom'}
									<span class="switch prestashop-switch switch-light codwfeeplus-switch-light {if isset($input.cols) && $input.cols}col-lg-{$input.cols}{else}col-lg-6{/if}">
										{foreach $input.values as $value}
										<input type="radio" name="{$input.name}" id="{if isset($value.id) && $value.id}{$value.id}{else}{$input.name}_{$value.value}{/if}" value="{$value.value}"{if $fields_value[$input.name] == $value.value} checked="checked"{/if}{if (isset($input.disabled) && $input.disabled) or (isset($value.disabled) && $value.disabled)} disabled="disabled"{/if}/>
										{strip}
										<label for="{if isset($value.id) && $value.id}{$value.id}{else}{$input.name}_{$value.value}{/if}">
											{$value.label}
										</label>
										{/strip}
										{/foreach}
